...
import welcome1 from "../../../public/assets/images/welcome_1.png";
import welcome2 from "../../../public/assets/images/welcome_2.jpg";
import welcome3 from "../../../public/assets/images/welcome_3.jpg";
...
function Welcome() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const clamped = Math.min(Math.max(scrollY - startScroll, 0), scrollRange);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="absolute flex w-[38%] bottom-[5%] left-[5%] bg-secondary shadow-xl rounded-sm max-md:flex-col max-md:w-[94%] max-md:mx-auto max-md:bottom-0 max-md:left-0 max-md:relative"
      >
        <div className="flex flex-col items-start py-[4%] px-[5%] bottom-[5%] left-[5%] bg-primary border-secondary border-l-[16px] max-md:border-l-0 max-md:pt-[8%] max-md:text-center max-md:items-center">
          ...
          {isMobile && (
            <motion.div className="mb-4" style={{ scale }}>
              <HugeiconsIcon icon={Yoga03Icon} size={44} strokeWidth={1.5} />
            </motion.div>
          )}
